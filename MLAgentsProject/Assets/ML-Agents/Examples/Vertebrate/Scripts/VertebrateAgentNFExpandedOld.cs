...
public class VertebrateAgentNFExtendedOld : Agent
{
    ...
    [Tooltip("Mnożnik kary za zużytą energię (sumaryczną siłę stawów)")]
...

    {
        ...
        TargetWalkingSpeed = 10;//Random.Range(0.1f, m_maxWalkingSpeed);
...

    {
        ...
        float currentSpineBendAngle = Vector3.SignedAngle(body.forward, segment2.forward, Vector3.up);

        float absoluteSpineBendAngle = Mathf.Abs(currentSpineBendAngle);



        // 3. Kara za pozostawanie w zgięciu dłużej niż X sekund

        if (absoluteSpineBendAngle > bentAngleThreshold)
...

        {
            ...
            // Resetujemy timer, jeśli agent się wyprostował
        ...
        // Sprawdzamy, jak bardzo agent musi skręcić, żeby iść do celu
...

        {
            ...
            // Jeśli agent musi skręcić (np. w lewo), a znak zgięcia kręgosłupa jest inny (odgina się w prawo) 

            // lub po prostu się nie zgina (wygięcie < 5 stopni), dostaje karę.

            if (Mathf.Sign(turnNeeded) != Mathf.Sign(currentSpineBendAngle) || absoluteSpineBendAngle < 5f)

            {

                AddReward(notBendingWhenTurningPenalty);
