...
from mlagents.trainers.td3.optimizer_torch_td3 import TorchTD3Optimizer
...
class TD3NoisePolicy(TorchPolicy):
    ...
    def evaluate(self, decision_requests, global_agent_ids: list) -> Dict[str, Any]:
        ...
        if action_tuple is not None and action_tuple.continuous is not None:
            ...
            # Wymuszamy akcje deterministyczne w zakresie [-1, 1]
            deterministic_actions = np.tanh(raw_actions)

            # 1. FAZA ROZGRZEWKI (Czysta losowość z OpenAI)
            if self.eval_step < self.warmup_steps:
                random_actions = np.random.uniform(low=-1.0, high=1.0, size=deterministic_actions.shape)
                run_out["action"] = ActionTuple(
                    continuous=np.float32(random_actions), 
                    discrete=action_tuple.discrete
                )
                self.eval_step += len(global_agent_ids) # Zliczamy po agentach, nie klatkach
                return run_out 

            # 2. FAZA NAUKI (Nieskorelowany szum Gaussa)
            ...
            # Dodajemy zwykły szum (np.randn jak w Spinning Up)
            noise = np.random.normal(loc=0.0, scale=current_noise_scale, size=deterministic_actions.shape)
            noisy_actions = deterministic_actions + noise
            
            # Ucinamy, żeby silnik fizyczny Unity nie dostał niemożliwych wartości
            ...
            self.eval_step += len(global_agent_ids)
...
class TD3Trainer(OffPolicyTrainer):
    ...
    def create_policy(
        ...
        policy = TD3NoisePolicy(
            ...
            actor_cls,
            actor_kwargs,
            start_noise=0.7,    # Parametry dla nowej nakładki TD3NoisePolicy (szum OU)
            end_noise=0.05,
            decay_steps=3000000,
            warmup_steps=50000
        )
